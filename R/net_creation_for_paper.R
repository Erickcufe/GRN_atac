...
write_graph(g_rorb, file = "Nets/Graph_RORB_PAPER_SFG.graphml",format = "graphml")
RORB_net <- RORB_net %>%
  select(TF, TG, coef, promoter, mse) %>%
  mutate(rmse = sqrt(mse)) %>%
  filter(rmse < mean(rmse))
RORB_net$edge_btw <- edge_betweenness(g_rorb)
saveRDS(RORB_net, "Nets/Net_RORB_PAPER_SFG.rds")
rm(so_morabito_RORB)
...
g_ex <- Ex_net %>%
  ...
  filter(rmse <= mean(rmse)) %>%
  graph_from_data_frame(directed = TRUE)
write_graph(g_ex, file = "Nets/Graph_Ex_PAPER_SFG.graphml",format = "graphml")
Ex_net <- Ex_net %>%
  select(TF, TG, coef, promoter, mse) %>%
  mutate(rmse = sqrt(mse)) %>%
  filter(rmse <= mean(rmse))
Ex_net$edge_btw <- edge_betweenness(g_ex)
saveRDS(Ex_net, "Nets/Net_Ex_PAPER_SFG.rds")
rm(so_morabito_Ex)
...
g_pv <- Pv_net %>%
  ...
  filter(rmse <= mean(rmse)) %>%
  graph_from_data_frame(directed = TRUE)
write_graph(g_pv, file = "Nets/Graph_Pv_PAPER_SFG.graphml",format = "graphml")
Pv_net <- Pv_net %>%
  select(TF, TG, coef, promoter, mse) %>%
  mutate(rmse = sqrt(mse)) %>%
  filter(rmse <= mean(rmse))
Pv_net$edge_btw <- edge_betweenness(g_pv)
saveRDS(Pv_net, "Nets/Net_Pv_PAPER_SFG.rds")
rm(so_morabito_Pv)
...
g_vip <- Vip_net %>%
  ...
  filter(rmse <= mean(rmse)) %>%
  graph_from_data_frame(directed = TRUE)
write_graph(g_vip, file = "Nets/Graph_Vip_PAPER_SFG.graphml",format = "graphml")
Vip_net <- Vip_net %>%
  select(TF, TG, coef, promoter, mse) %>%
  mutate(rmse = sqrt(mse)) %>%
  filter(rmse <= mean(rmse))
Vip_net$edge_btw <- edge_betweenness(g_vip)
saveRDS(Pv_net, "Nets/Net_Vip_PAPER_SFG.rds")
rm(so_morabito_Vip)
...
g_non_vip <- Non_Vip_net %>%
  ...
  filter(rmse <= mean(rmse)) %>%
  graph_from_data_frame(directed = TRUE)
write_graph(g_non_vip, file = "Nets/Graph_Non_Vip_PAPER_SFG.graphml",format = "graphml")
Non_Vip_net <- Non_Vip_net %>%
  select(TF, TG, coef, promoter, mse) %>%
  mutate(rmse = sqrt(mse)) %>%
  filter(rmse <= mean(rmse))
Non_Vip_net$edge_btw <- edge_betweenness(g_non_vip)
saveRDS(Pv_net, "Nets/Net_Non_Vip_PAPER_SFG.rds")
rm(so_morabito_Non_Vip)
...
g_sst <- Sst_net %>%
  ...
  filter(rmse <= mean(rmse)) %>%
  graph_from_data_frame(directed = TRUE)
write_graph(g_sst, file = "Nets/Graph_Sst_PAPER_SFG.graphml",format = "graphml")
Sst_net <- Sst_net %>%
  select(TF, TG, coef, promoter, mse) %>%
  mutate(rmse = sqrt(mse)) %>%
  filter(rmse <= mean(rmse))
Sst_net$edge_btw <- edge_betweenness(g_sst)
saveRDS(Pv_net, "Nets/Net_Sst_PAPER_SFG.rds")
rm(so_morabito_Sst)
...
g_inh <- inh_net %>%
  ...
  filter(rmse < mean(rmse)) %>%
  graph_from_data_frame(directed = TRUE)
write_graph(g_inh, file = "Nets/Graph_INH_PAPER_SFG.graphml", format = "graphml")
inh_net <- inh_net %>%
  select(TF, TG, coef, promoter, mse) %>%
  mutate(rmse = sqrt(mse)) %>%
  filter(rmse < mean(rmse))
inh_net$edge_btw <- edge_betweenness(g_inh)
saveRDS(Pv_net, "Nets/Net_Inh_PAPER_SFG.rds")
rm(so_morabito_Inh)
